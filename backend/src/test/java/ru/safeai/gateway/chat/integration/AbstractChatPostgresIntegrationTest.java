...
abstract class AbstractChatPostgresIntegrationTest
    ...
    void prepareChatFixture() {
        ...
        insertOrganization(ORGANIZATION_ID, "Chat Organization", true);
        insertOrganization(OTHER_ORGANIZATION_ID, "Other Organization", true);
        insertUser(
                USER_ID,
                ORGANIZATION_ID,
        ...
        insertChatSession(CHAT_ID, USER_ID, ORGANIZATION_ID);
        insertChatSession(
                OTHER_CHAT_ID,
                OTHER_USER_ID,
                OTHER_ORGANIZATION_ID
    ...
    protected UUID insertCompletedAssistant(
        ...
        jdbcTemplate.update("""
                        insert into public.chat_messages (
                            ...
                            ?, ?, ?, 'ASSISTANT', ?, ?,
                            'mock-safeai', 'mock-safeai', ?, ?, ?, 'completed',
                            10, 20, 'AVAILABLE', 0, 'FREE', 'USD',
                            'mock-2026-01', ?, ?, 'COMPLETED'
                        )
                        """,
                messageId,
                sessionId,
