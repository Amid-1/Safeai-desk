...
class AuditOutboxProcessorTest {
    ...
    void copiesEventBeforeDeletingOutboxRow() {
        AuditOutboxEntity item = item();

        whenNextRows(item);

        when(eventRepository.insertFromOutbox(
                item.getId()
        )).thenReturn(1);
    ...
    void alreadyDeliveredEventIsTreatedAsIdempotentSuccess() {
        ...
        verify(outboxRepository).delete(item);
        verify(outboxRepository).flush();
    }

    @Test
    void poisonRowGetsBackoffAndDoesNotBlockNextRow() {
        AuditOutboxEntity poison = item();
        AuditOutboxEntity valid = item();

        whenNextRows(poison, valid);
        ...
        when(failureService.markFailure(
                eq(poison.getId()),
        ...
        verify(eventRepository)
                .insertFromOutbox(valid.getId());

        verify(outboxRepository).delete(valid);
    }
    ...
    void commitFailureIsCaughtAndFailurePolicyIsApplied() {
        ...
        verify(failureService).markFailure(
                eq(item.getId()),
                any(RuntimeException.class)
        );
    }
    ...
    private void whenNextRows(
        ...
        OngoingStubbing<Optional<AuditOutboxEntity>>
                stubbing = when(
                outboxRepository.findNextForUpdate(NOW)
        );
