...
public final class DatabaseConstraintClassifier {
    ...
    public static boolean isUniqueViolation(
        ...
        if (!hasSqlState(throwable, UNIQUE_VIOLATION)) {
            return false;
        }

        Set<String> accepted = Set.of(acceptedConstraintNames);
        String actualConstraint = findConstraintName(throwable);

        return actualConstraint != null
                && accepted.contains(actualConstraint);
    }

    public static boolean isForeignKeyViolation(Throwable throwable) {
        return hasSqlState(throwable, FOREIGN_KEY_VIOLATION);
    }
    ...
    private static boolean hasSqlState(
        ...
        for (Throwable current : causeChain(throwable)) {
            ...
            if (current instanceof ConstraintViolationException violation
                    && expectedSqlState.equals(violation.getSQLState())) {
                return true;
            }
        }
    ...
    private static Set<Throwable> causeChain(Throwable throwable) {
