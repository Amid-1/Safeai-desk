...
public class UserService {
    ...
    private final UserManagementProperties
    ...
    public UserResponse create(
         ...
         *
         * Она не является correctness boundary:
         * race всё равно закрывается UNIQUE constraint.
         */
        ...
        /*
         * BCrypt cost=12 достаточно дорогой.
         *
         * Хешируем до захвата organization PESSIMISTIC_WRITE,
         * чтобы не держать DB lock во время CPU-intensive BCrypt.
         */
        String passwordHash =
        ...
        try {
            /*
             * saveAndFlush намеренно.
             *
             * Constraint violation должен произойти
             * внутри этого try/catch, а не только
    ...
    public UserResponse updateRoles(
        ...
        requireExactlyOneRequestedRole(
                request.roles()
        );
    ...
    public void resetPassword(
         ...
         * Service может вызываться напрямую из другого слоя.
         */
        String password =
                requireValidNewPassword(
                        request.password()
    ...
    public void permanentlyDelete(
        ...
        if (!isSuperAdmin(currentUser)) {
            throw new ForbiddenOperationException(
                    "Только SUPER_ADMIN может "
                            + "удалять пользователей навсегда"
            );
        }
        ...
        if (user.isEnabled()) {
            throw new ForbiddenOperationException(
                    "Перед окончательным удалением "
                            + "пользователя необходимо отключить"
            );
        }
        ...
        Instant deletionAllowedAt =
                disabledAt.plus(
                        userManagementProperties
                                .permanentDeletionRetention()
                );
    ...
    private OrganizationEntity lockOrganization(
            UUID organizationId
    ) {
        OrganizationEntity organization =
                entityManager.find(
    ...
    private void assertNotLastEnabledAdmin(
            UserEntity user
    ) {
    ...
    private void publishSecurityStateChanged(
        ...
        eventPublisher.publishEvent(
                new UserSecurityStateChangedEvent(
                        userId
                )
        );
    }
     ...
     * Defense in depth.
     *
     * <p>DTO Bean Validation не считается единственной
     * security boundary: service может быть вызван напрямую.</p>
     */
    private String requireValidNewPassword(
            String password
    ) {
        if (!PasswordPolicy
                .isValidNewPassword(
    ...
    private boolean isEmailUniqueViolation(
            Throwable exception
    ) {
