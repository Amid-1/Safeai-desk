...
public class ModelCatalogService {
    ...
    public List<ModelCatalogEntryResponse> findLatest(SafeAiUserPrincipal currentUser) {
        ModelControlPlaneAccess.requireAdminOrSuperAdmin(
                currentUser,
                "Недостаточно прав для просмотра model catalog"
        );
        return repository.findLatestAll().stream()
                .map(ModelCatalogEntryResponse::from)
                .toList();
    }

    /**
     * Returns the catalog snapshot that is effective at the server-controlled
     * current time. Future-scheduled versions remain visible through
     * {@link #findLatest(SafeAiUserPrincipal)} but cannot become executable
     * before their effective_from instant.
     */
    @Transactional(readOnly = true)
    public List<ModelCatalogEntryResponse> findEffective(SafeAiUserPrincipal currentUser) {
        ModelControlPlaneAccess.requireAdminOrSuperAdmin(
                currentUser,
                "Недостаточно прав для просмотра model catalog"
        );
        Instant now = clock.instant();
        return repository.findEffectiveAll(now).stream()
                .map(ModelCatalogEntryResponse::from)
                .toList();
    }
    ...
    public ModelCatalogEntryResponse createVersion(
        ...
        String modelKey = normalizeModelKey(request.modelKey());
        String provider = ModelCatalogRules.normalizeProvider(request.provider());
        String providerModelId = ModelCatalogRules.normalizeRequired(request.providerModelId(), 100, "providerModelId");
        String displayName = ModelCatalogRules.normalizeRequired(request.displayName(), 255, "displayName");
        String extraPricingJson = ModelCatalogRules.validateExtraPricingJson(request.extraPricingJson());
        String pricingVersion = ModelCatalogRules.normalizePricingVersion(request.pricingVersion());
        BigDecimal inputPrice = ModelCatalogRules.normalizeMoney(
                request.inputUsdPer1mTokens(),
                "inputUsdPer1mTokens"
        );
        BigDecimal cachedInputPrice = ModelCatalogRules.normalizeMoney(
                request.cachedInputUsdPer1mTokens(),
                "cachedInputUsdPer1mTokens"
        );
        BigDecimal cacheWriteInputPrice = ModelCatalogRules.normalizeMoney(
                request.cacheWriteInputUsdPer1mTokens(),
                "cacheWriteInputUsdPer1mTokens"
        );
        BigDecimal outputPrice = ModelCatalogRules.normalizeMoney(
                request.outputUsdPer1mTokens(),
                "outputUsdPer1mTokens"
        );

        Set<ModelCapability> capabilities = request.capabilities() == null
                ? Set.of()
                : Set.copyOf(request.capabilities());
        Set<ModelModality> inputModalities = request.inputModalities() == null
                ? Set.of(ModelModality.TEXT)
                : Set.copyOf(request.inputModalities());
        Set<ModelModality> outputModalities = request.outputModalities() == null
                ? Set.of(ModelModality.TEXT)
        ...
        int previousVersion = repository.findLatest(modelKey)
        ...
        ModelCatalogEntry entry = new ModelCatalogEntry(
                ...
                extraPricingJson,
                pricingVersion,
                request.effectiveFrom() == null ? now : request.effectiveFrom(),
     ...
     * missing retention/training/specialized-pricing metadata.
     */
    @Transactional
    public ModelCatalogEntryResponse importRuntime(
        ...
        String runtimeProvider = ModelCatalogRules.normalizeProvider(
                runtime.provider()
        );
        String runtimeModel = ModelCatalogRules.normalizeRequired(
                runtime.model(),
                100,
                "runtime.model"
        );
        String modelKey = normalizeModelKey(
                runtimeProvider + ":" + runtimeModel
        );

        String runtimePricingStatus = Objects.requireNonNull(
        ...
        switch (runtimePricingStatus) {
            ...
            case "CONFIGURED" -> {
                // Legacy flat config has no cached/cache-write/extra dimensions.
        ...
        BigDecimal runtimeInputPrice = ModelCatalogRules.normalizeMoney(
                runtime.inputUsdPer1mTokens(),
                "runtime.inputUsdPer1mTokens"
        );
        BigDecimal runtimeOutputPrice = ModelCatalogRules.normalizeMoney(
                runtime.outputUsdPer1mTokens(),
                "runtime.outputUsdPer1mTokens"
        );
        String runtimePricingVersion =
                ModelCatalogRules.normalizePricingVersion(runtime.pricingVersion());
